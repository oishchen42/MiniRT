...
/*   Updated: 2025/12/14 16:37:52 by oishchen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//{
	...
	////create_transform_mtx4(right_wall, &res_1mtx);
	////wadd_obj(&wrld, NULL, right_wall);
	//t_obj	*cl = cylinder(NULL, NULL, 1.0, 3.0);
	//t_vcpnt	mv = {3, 0, 1, 1};
	...
	int main()
	{
		printf("MOVING camera in z and x direction\n");
		t_master	app;

		if (!init_master(&app))
			return (printf("Error: master init\n"), 1);
	}
//}
