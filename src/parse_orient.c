...
/*   Updated: 2025/12/16 21:10:14 by tdietz-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_mtx4	get_rotation_matrix(t_vcpnt orientation)
{
	...
	get_id_mtx4(&mtx);
	fill_matrix_cols(&mtx, new_x, new_y, new_z);
	return (mtx);
}

// Timo updated function. Excluded matrix filling to separate function for norm
static void	fill_matrix_cols(t_mtx4 *mtx, t_vcpnt x, t_vcpnt y, t_vcpnt z)
{
	// First Column (X axis)
...
void	pr_cm_orient(t_camera *cam, t_vcpnt pos, t_vcpnt dir, double par_fov_d)
{
	t_vcpnt	parsed_pos;
	t_vcpnt	parsed_dir;
	double	parsed_fov_deg;

	parsed_pos = {-50.0, 0, 20, 1};
	parsed_dir = {0, 0, 1, 0};
	parsed_fov_deg = 70.0;
	cam->from = parsed_pos;
